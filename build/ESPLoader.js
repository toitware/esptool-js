...
class ESP32S2ROM {
  ...
  static read_mac = async loader => {
    var mac0 = await loader.read_reg({
      addr: this.MAC_EFUSE_REG
    });
    mac0 = mac0 >>> 0;
    var mac1 = await loader.read_reg({
      addr: this.MAC_EFUSE_REG + 4
    });
...
class ESPLoader {
  ...
  ESP_READ_REG = 0x0A;
  ESP_SPI_ATTACH = 0x0D;
  ESP_CHANGE_BAUDRATE = 0x0F; // Only Stub supported commands
